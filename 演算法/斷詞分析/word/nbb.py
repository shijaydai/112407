...
# 選取特定店家的評論
selected_store_comments = df[df['storeid'] == selected_store_id]['segmented']

# 使用訓練好的模型進行預測
selected_store_comments_vectorized = vectorizer.transform(selected_store_comments)
predictions = clf.predict(selected_store_comments_vectorized)

# 顯示預測結果
results = pd.DataFrame({'Comment': selected_store_comments, 'Prediction': predictions})

# 匯出預測結果為 CSV 檔案，使用 mode='a' 附加到現有檔案，header=False 避免寫入標題
results.to_csv('Gappdj_finishh.csv', mode='a', header=False, index=False, encoding='cp950')
